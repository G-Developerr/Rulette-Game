...
function labelFor(type, value) {
  ...
  if (type==='sector') {
    if (value==='voisins') return 'Voisins';
    if (value==='zero') return 'Zero';
    if (value==='tier') return 'Tier';
    if (value==='orphans') return 'Orphans';
  }
  return '';
}
...
function checkWin(type, value, n) {
  ...
  if (type==='sector') {
    const nums = SECTOR_NUMS[value];
    return nums ? nums.has(n) : false;
  }
  return false;
}
function getSectorNumbers(value) {
  const nums = SECTOR_NUMS[value];
  return nums ? [...nums] : [];
}
function sectorShort(value) {
  if (value === 'voisins') return 'V';
  if (value === 'zero') return 'Z';
...
function refreshSectorCoverage() {
  document.querySelectorAll('.bc[data-type="number"]').forEach((cell) => {
    cell.classList.remove('sector-covered');
    cell.querySelector('.sector-cover')?.remove();
  });

  const map = new Map();
  activeBets
    .filter((b) => b.type === 'sector')
    .forEach((b) => {
      getSectorNumbers(b.value).forEach((n) => {
        const labels = map.get(n) || [];
        labels.push(sectorShort(b.value));
        map.set(n, labels);
      });
    });

  map.forEach((labels, n) => {
    const cell = document.querySelector(`.bc[data-type="number"][data-value="${n}"]`);
    if (!cell) return;
    cell.classList.add('sector-covered');
    const cover = document.createElement('div');
    cover.className = 'sector-cover';
    cover.textContent = labels.join('/');
...
function buildTable() {
  ...
  document.querySelectorAll('.bc[data-type]:not([data-type="number"])').forEach(cell=>{
    ...
    const pay=payLbl(cell.dataset.type,cell.dataset.value);
